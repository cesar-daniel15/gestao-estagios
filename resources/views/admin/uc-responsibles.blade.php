    ...
    <div class="flex flex-col gap-5 md:flex-row justify-between items-center my-5">
        ...
        <div class="relative w-full lg:w-72 mb-4 md:mb-0">
            <input type="text" id="search" class="bg-gray-50 border border-gray-300 text-black text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block pl-10 w-full p-2.5 text-start" 
                placeholder="Procurar por Responsável de UC" oninput="searchResponsible()" />
        ...
        <table class="table-auto w-full border-collapse text-center text-sm overflow-hidden rounded-xl hidden" id="responsibleTable">
            <thead>
                <tr class="bg-gray-100">
                    ...
                    <th class="p-4 border-b text-gray-600">Nome de Responsável</th>
                    <th class="p-4 border-b text-gray-600">Contacto</th>
                    <th class="p-4 border-b text-gray-600">Foto</th>
            ...
            @if (empty($UcResponsibles))
                <tr>
                    <td colspan="7" class="p-4 text-gray-600 text-center">Ainda não existem responsáveis registados </td>
                </tr>
            @else
                @foreach($UcResponsibles as $UcResponsible)
                    <tr class="border-b hover:bg-gray-50">
                        <td class="p-4 text-gray-600">{{ $UcResponsible['id'] }}</td>
                        <td class="p-4 text-gray-600">{{ $UcResponsible['name'] ?? 'Nome não disponível' }}</td>
                        <td class="p-4 text-gray-600">{{ $UcResponsible['contact'] }}</td>
                        <td class="p-4 text-gray-600">{{ $UcResponsible['photo'] }}</td>
                        <td class="p-4 text-gray-600">
                            <div class="flex space-x-2 justify-center">
                                
                                <!-- Botão Ver -->
                                <a onclick="viewModal({{ $UcResponsible['id'] }},'{{ $UcResponsible['name'] }}', '{{ $UcResponsible['institution']['name'] }}', '{{ $UcResponsible['contact'] }}')" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-2 rounded flex items-center">
                                ...
                                <!-- Botão Atualizar -->
                                <button type="button" onclick="updateModal({{ $UcResponsible['id'] }}, '{{ $UcResponsible['name'] }}', '{{ $UcResponsible['contact'] }}')" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-2 rounded flex items-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-5 h-5">
                                ...
                                <form id="deleteForm{{ $UcResponsible['id'] }}" action="{{ route('admin.uc_responsibles.destroy', $UcResponsible['id']) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" onclick="openDeleteModal({{ $responsible['id'] }})" class="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-2 rounded flex items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-5 h-5">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body flex flex-col md:flex-row">
                ...
                <div class="ml-4 space-y-4 flex flex-col data-content">
                    ...
                    <p><strong>Foto:</strong> <span id="modal-photo"></span> </p>
                </div>
            </div>
...
<div id="updateModal" class="fixed inset-0 items-center sm:h-screen justify-center z-50 bg-black bg-opacity-50 hidden text-sm">
    <div class="bg-white rounded-lg shadow-lg p-6 w-11/12 sm:w-3/4 md:w-2/3 lg:w-1/2">
        ...
        <!-- Form -->
        <form id="updateForm" action="{{ route('admin.uc_responsibles.update', $institution['id']) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
...
<script>
    ...
    // Funcao para modal de view dos responsáveis
    function viewModal(id, name, institution, phone, createdAt) {
    ...
    // Abrir Modal para fazer uma atualizacao de um Responsável
