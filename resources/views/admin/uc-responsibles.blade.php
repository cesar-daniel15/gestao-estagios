    ...
    <div class="flex flex-col gap-5 md:flex-row justify-between items-center my-5">
        ...
        <div class="flex gap-4">
            ...
            <div class="mt-4 md:mt-0 hidden md:flex">
                <button id="refreshButton" class="bg-info text-white font-bold p-2 rounde bg-teal-600 hover:bg-teal-500 rounded-lg flex items-center" onclick="refreshTable()">
            ...
            @else
                @foreach($UcResponsibles as $UcResponsible)
                    <tr class="border-b hover:bg-gray-50">
                        ...
                        <td class="p-4 text-gray-600 responsible-name">{{ $UcResponsible['name'] ?? 'Nome não disponível' }}</td>
                        <td class="p-4 text-gray-600">{{ $UcResponsible['phone'] }}</td>
                        <td class="p-4 text-gray-600">
                            <div class="flex space-x-2 justify-center">
                                
                                <!-- Botão Ver -->
                                <a onclick="viewModal({{ $UcResponsible['id'] }}, '{{ $UcResponsible['phone'] }}', '{{ $UcResponsible['picture'] }}')" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-2 rounded flex items-center">
                                ...
                                <form id="deleteForm{{ $UcResponsible['id'] }}" action="{{ route('admin.uc_responsibles.destroy', $UcResponsible['id']) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <!-- O botão de exclusão que chama a função openDeleteModal -->
    ...
    <div class="bg-white p-6 rounded-lg relative w-11/12 sm:w-3/4 md:w-2/3 lg:w-1/2">
        <div class="modal-content flex flex-col">
            ...
            <div class="modal-body flex flex-col md:flex-row">
                <!-- Div para a imagem -->
                <div class="w-full md:w-1/2 p-4 flex justify-center items-center" id="modal-picture-container">
                    <img id="modal-picture" src="" alt="Foto do Responsável" class="w-full h-48 object-contain rounded-2xl">
                </div>
...
<script>
    ...
    function viewModal(id, name, phone, createdAt, picture) {
    ...
    function updateModal(id, name, phone, picture) {
        ...
        // Exibindo a foto atual no modal
    ...
    function searchUcResponsible() {
        ...
        const rows = document.querySelectorAll("#ucResponsibleTable tbody tr");

        rows.forEach(row => {
            const responsibleName = row.querySelector(".responsible-name").textContent.toLowerCase();
            const ucName = row.querySelector(".uc-name").textContent.toLowerCase();
            
            // Exibe a linha se o nome do responsável ou da UC incluir o valor pesquisado
            if (responsibleName.includes(searchValue) || ucName.includes(searchValue)) {
                row.style.display = "";
            } else {
