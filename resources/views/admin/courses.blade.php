        ...
        <div class="flex flex-col gap-5 md:flex-row justify-between items-center my-5">
            ...
            <div class="flex flex-col md:flex-row">
                ...
                <div class="relative w-full md:w-auto mb-4 md:mb-0">
                    <input type="text" id="search-course" class="bg-gray-50 border border-gray-300 text-black text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block pl-10 w-full p-2 text-start" placeholder="Procurar por Curso" oninput="searchCourse()" />
                ...
                <div class="relative w-auto md:ms-5 mt-4 md:mt-0">
                    <select id="search-institution" class="bg-gray-50 border border-gray-300 text-gray-500 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block pl-10 w-full p-2 text-start" onchange="searchCourse()">
                ...
                @foreach ($courses as $course)
                    <tr class="border-b hover:bg-gray-50">
                        ...
                        <div class="flex space-x-2 justify-center">
                            ...
                            <!-- Botão Ver -->
                            <a  onclick="viewModal({{ $course['id'] }},'{{ $course['name'] }}', '{{ $course['acronym'] }}', '{{ $course['institution']['acronym'] }}', '{{ $course['created_at'] }}')" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-2 rounded flex items-center">
                            ...
                            <!-- Botão Update -->
                            <button type="button" onclick="updateModal({{ $course['id'] }}, '{{ $course['name'] }}', '{{ $course['acronym'] }}', '{{ $course['institution']['id'] }}')" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-2 rounded flex items-center">                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-5 h-5">
            ...
            <form id="updateForm" method="POST">                
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <select id="update_institution_id" name="institution_id" class="form-input mt-1 block w-full border rounded-md py-2 px-4">
                            @foreach($institutions as $institution)
                                <option value="{{ $institution['id'] }}">{{ $institution['acronym'] }}</option>
                            @endforeach
                        </select>
    ...
    <script>
        ...
        function searchCourse() {
            ...
            rows.forEach(row => {
                ...
                const courseInstitution = row.querySelector(".course-institution").textContent;

                const matchName = courseName.includes(searchCourse);
                const matchInstitution = searchInstitution ? courseInstitution === searchInstitution : true;

                if (searchInstitution && searchCourse) {
                    if (matchInstitution && matchName) {
                        row.style.display = ""; 
                    } else {
                        row.style.display = "none"; 
                    }
                } else if (searchInstitution) {
                    if (matchInstitution) {
                        row.style.display = ""; 
                    } else {
                        row.style.display = "none"; 
        ...
        function viewModal(id, name, acronym, institution_id, created_at) {
            ...
            document.querySelector('#viewModal .modal-body .data-content').innerHTML = `
                <div class="ml-4 flex flex-col gap-5">
                    ...
                    <p><strong>Instituição:</strong> ${institution_id}</p>
