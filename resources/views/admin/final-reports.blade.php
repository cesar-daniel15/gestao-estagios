...
@section('page-name', 'Estágios / Relatórios Finais')
...
@include('admin.layouts.components.alert')
                        ...
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="p-4 text-gray-600">
                                <div class="flex space-x-2 justify-center">
                                    ...
                                    <!-- Botão Ver -->
                                    <a onclick="viewModal({{ $final_report['id'] }}, '{{ $final_report['total_hours'] }}', '{{ $final_report['total_days'] }}', '{{ $final_report['final_report_content'] }}', '{{ $final_report['company_evaluation'] }}', '{{ $final_report['final_evaluation'] }}', '{{ $final_report['status'] }}', '{{ $final_report['attachment'] }}', '{{ $final_report['created_at'] }}')" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-2 rounded flex items-center">
    ...
    <div id="createModal" class="fixed inset-0 items-center sm:h-screen justify-center z-50 bg-black bg-opacity-50 hidden text-sm">
        <div class="bg-white rounded-lg shadow-lg p-6 w-11/12 sm:w-3/4 md:w-2/3 lg:w-1/2">
            <h2 class="text-xl font-bold text-gray-700 mb-4 text-center">Registrar Relatório Final</h2>

            <!-- Form -->
            <form action="{{ route('admin.internship_final_reports.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
    ...
    <div id="updateModal" class="fixed inset-0 items-center sm:h-screen justify-center z-50 bg-black bg-opacity-50 hidden text-sm">
        <div class="bg-white rounded-lg shadow-lg p-6 w-11/12 sm:w-3/4 md:w-2/3 lg:w-1/2">
            <h2 class="text-xl font-bold text-gray-700 mb-4 text-center">Atualizar Relatório Final</h2>

            <!-- Form -->
            <form id="updateForm" action="{{ route('admin.internship_final_reports.update', $final_report['id']) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <select id="update_status" name="status" class="border border-gray-300 rounded-lg w-full p-1 xl:p-2">
                            <option value="Pendente" {{ $final_report['status'] == 'Pendente' ? 'selected' : '' }}>Pendente</option>
                            <option value="Aprovado" {{ $final_report['status'] == 'Aprovado' ? 'selected' : '' }}>Aprovado</option>
                            <option value="Rejeitado" {{ $final_report['status'] == 'Rejeitado' ? 'selected' : '' }}>Rejeitado</option>
                        </select>
                    </div>
    ...
    <div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-50 items-center justify-center z-50 hidden">
        <div class="bg-white p-6 rounded-lg w-1/3">
            <h2 class="text-md font-semibold py-10 text-center text-gray-500">Tem a certeza que deseja apagar este relatório final?</h2>
            ...
            <div class="flex justify-center space-x-5">
                ...
                <form id="deleteForm" action="{{ route('admin.internship_final_reports.destroy', $final_report['id']) }}" method="POST">
                    @csrf
                    @method('DELETE')
                    <button type="submit" id="confirmDeleteButton" class="bg-red-500 hover:bg-red-600 text-white p-2.5 font-bold rounded">Confirmar</button>
                </form>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function updateModal(id, totalHours, totalDays, content, companyEvaluation, finalEvaluation, status, attachment) {
            ...
            updateForm.action = `/admin/final_reports/${id}`;
        }
