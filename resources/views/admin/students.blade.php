        ...
        <div class="flex flex-col gap-5 md:flex-row justify-between items-center my-5">
            ...
            <div class="relative w-full md:w-auto mb-4 md:mb-0">
                <input type="text" id="search" class="bg-gray-50 border border-gray-300 text-black text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block pl-10 w-full p-2 text-start" 
                    placeholder="Procurar por Instituição" oninput="searchStudent()" />
                ...
                @else
                    @foreach($students as $student)
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="p-4 text-gray-600">{{ $student->name }}</td>
                            <td class="p-4 text-gray-600">{{ $student->phone }}</td>
                            <td class="p-4 text-gray-600">
                                <img src="{{ $student->photo }}" alt="Foto de {{ $student->name }}" class="h-10 w-10 object-cover rounded-full">
                            </td>
                            <td class="p-4 text-gray-600">
                                {{ $student->assignedInternship->name ?? 'Sem Estágio' }} <!-- Nome do estágio ou fallback -->
                            </td>
                            <td class="p-4 text-gray-600">
                                <div class="flex space-x-2 justify-center">
                                        
                                    <!-- Botão Ver -->
                                    <a onclick="viewModal({{ $student['id'] }},'{{ $student['user']['name'] }}','{{ $student['phone'] }}', '{{ $student['picture'] }}', '{{ $student['created_at'] }}', '{{ $student['assignedInternship']['name'] }}')" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-2 rounded flex items-center">
                                    ...
                                    <!-- Botão Update -->
                                    <button type="button" onclick="updateModal({{ $student['id'] }}, '{{ $student['phone'] }}', '{{ $institution['assignedInternship']['name'] }}')" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-2 rounded flex items-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-5 h-5">
