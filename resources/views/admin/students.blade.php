        ...
        <div class="flex flex-col gap-5 md:flex-row justify-between items-center my-5">
            ...
            <div class="relative w-full md:w-auto mb-4 md:mb-0">
                <input type="text" id="search" class="bg-gray-50 border border-gray-300 text-black text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block pl-10 w-full p-2 text-start" 
                    placeholder="Procurar por Aluno" oninput="searchStudent()" />
                ...
                @else
                    @foreach($students as $student)
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="p-4 text-gray-600">{{ $student['user']['name']  }}</td>
                            <td class="p-4 text-gray-600">{{ $student['phone'] }}</td>
                            <td class="p-4 text-gray-600">
                                {{ $student['assignedInternship'] ?? 'Sem Estágio' }} <!-- Nome do estágio ou fallback -->
                            </td>
                            <td class="p-4 text-gray-600">
                                <div class="flex space-x-2 justify-center">
                                        
                                    <!-- Botão Ver -->
                                    <a onclick="viewModal({{ $student['id'] }},'{{ $student['user']['name'] }}','{{ $student['phone'] }}', '{{ $student['picture'] }}','{{ $student['created_at'] }}','{{ $student['updated_at'] }}', '{{ $student['assignedInternship']['name'] ?? 'Sem Estágio' }}')" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-2 rounded flex items-center">
                                    ...
                                    <!-- Botão Associar -->
                                    <button type="button" onclick="" class="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-2 rounded flex items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-5 h-5">
                                    ...
                                    <!-- Botão Apagar -->
                                    <form id="deleteForm{{ $student['id'] }}" action="{{ route('admin.students.destroy', $student['id']) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
            ...
            <div class="modal-content flex flex-col">
                ...
                <div class="modal-body flex flex-col md:flex-row">
                    ...
                    <div class="w-full md:w-1/2 p-4 flex justify-center items-center" id="modal-picture">
                        <!-- A imagem será inserida dinamicamente pelo JS -->
                    </div>
                    ...
                    <div class="w-full md:w-1/2 p-4 flex flex-col data-content space-y-4">
                        ...
                        <p><strong>Data de Atualização:</strong> <span id="modal-updated_at"></span></p>
                    </div>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 w-11/12 sm:w-3/4 md:w-2/3 lg:w-1/2">
            ...
            <form id="updateForm" action="{{ route('admin.students.update', $student['id']) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        <label for="picture" class="block text-gray-600 mb-1">Foto</label>
                        <input type="file" id="update_picture" name="picture" class="border border-gray-300 rounded-lg w-full p-1 xl:p-2">
                    </div>
...
@endif
    ...
    <script>
        ...
        function viewModal(id, name, phone, picture, createdAt,updatedAt, internship) {
            ...
            document.querySelector('#viewModal .modal-body .data-content').innerHTML = `
                <div class="ml-4 flex flex-col gap-5">
                    ...
                    <p><strong>Data de Atualização:</strong> ${updatedAt}</p>
                </div>
            `;
        ...
        function updateModal(id, phone, internship, picture) {
            openModal('updateModal');  
            
            document.getElementById('update_phone').value = phone;
            // document.getElementById('assigned_internship_id').value = internship;

            const currentPicture = document.getElementById('current_picture');
            currentPicture.src = picture ? picture : 'path_to_default_image';  // Se não houver imagem, usa uma imagem padrão
       ...
       function searchStudent() {
            ...
            rows.forEach(row => {
                // Recuperar o nome e o telefone do estudante
                const studentName = row.querySelector(".student-name").textContent.toLowerCase();
                const studentPhone = row.querySelector(".student-phone").textContent.toLowerCase();
                
                // Verificar se o nome OU o telefone contém o valor da pesquisa
                if (studentName.includes(searchValue) || studentPhone.includes(searchValue)) {
