                        ...
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="p-4 text-gray-600">
                                <div class="flex space-x-2 justify-center">
                                    ...
                                    <!-- Botão Ver -->
                                    <a onclick="viewModal({{ $student['id'] }}, '{{($student['user']['name']) }}', '{{ $student['phone'] }}', '{{ asset($student['picture']) }}', '{{ $student['assigned_internship_id'] }}', '{{ $student['created_at'] }}')" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-2 rounded flex items-center">                                        
                                    ...
                                    <!-- Botão Associar -->
                                    <button type="button" onclick="associateModal({{ $student['id'] }})"  class="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-2 rounded flex items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-5 h-5">
...
<div id="createModal" class="fixed inset-0 items-center sm:h-screen justify-center z-50 bg-black bg-opacity-50 hidden text-sm">
    ...
    <div id="associateModal" class="fixed inset-0 items-center sm:h-screen justify-center z-50 bg-black bg-opacity-50 hidden text-sm">
        <div class="bg-white rounded-lg shadow-lg p-6 w-11/12 sm:w-3/4 md:w-2/3 lg:w-1/2">
            ...
            <!-- Form -->
            <form id="associateForm" action="{{ route('admin.students.associateStudentToUc', $student['id']) }}" method="POST">
                @csrf

                <!-- Seleção da Unidade Curricular -->
                <div class="mb-4">
                    <label for="uc_id" class="block text-gray-600 mb-1">Selecione a uma Unidade Curricular</label>
                    <select id="uc_id" name="uc_id" class="border border-gray-300 rounded-lg w-full p-1 xl:p-2">
                        <option>Selecione a Unidade Curricular</option>
