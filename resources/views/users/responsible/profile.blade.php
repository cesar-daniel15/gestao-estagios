...
@section('content')
    ...
    <div class="mt-10 bg-white drop-shadow-md rounded-xl p-10">
        ...
        <form action="{{ route('responsible.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="flex flex-col lg:flex-row gap-6">
                ...
                <div class="flex-1 flex flex-col items-center justify-center border border-gray rounded-lg text-sm p-5">
                    <label for="picture" class="block text-gray-600 mb-4">Foto de Perfil</label>
                    <div class="w-48 h-48 border border-gray-300 rounded-lg flex items-center justify-center mb-4">
                        @if ($responsible->picture ?? false)
                            <img src="{{ asset('storage/' . $responsible->picture) }}" alt="Foto Atual" class="w-full h-full object-contain rounded-lg">
                        @else
