    ...
    <div class="mt-10 bg-white drop-shadow-md rounded-xl p-10 mb-10">
        ...
        <div class="flex flex-col md:flex-row">
            ...
            <div class="relative w-full md:w-auto mb-4 md:mb-0">
                <input type="text" id="search" class="bg-gray-50 border border-gray-300 text-black text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block pl-10 w-full p-2 text-start" placeholder="Procurar por Título" oninput="search()" />
                    ...
                    @else
                        @foreach($students as $student)
                            <tr class="border-b hover:bg-gray-50">
                                <td class="p-4 text-gray-600 student-title">
                                        {{ $student['user']['name'] }}
                                ...
                                <td class="p-4 text-gray-600">
                                    ...
                                    @if (isset($student['internship_offer']['id']) && !empty($student['internship_offer']['final_reports']['id'])) 
                                        <a href="{{ route('responsible.export.internship.final', ['studentId' => $student['id'], 'finalReportId' => $student['internship_offer']['final_report']['id']]) }}" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-2 rounded flex items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-5 h-5">
                                                <path fill="currentColor" d="m12 16l-5-5l1.4-1.45l2.6 2.6V4h2v8.15l2.6-2.6L17 11zm-6 4q-.825 0-1.412-.587T4 18v-3h2v3h12v-3h2v3q0 .825-.587 1.413T18 20z"/>
                                            </svg>
                                        </a>
                                    @else
                                        <span>Relatório indisponivel</span>
                                    @endif
                                    </div>
                                </td>
                            </tr>
