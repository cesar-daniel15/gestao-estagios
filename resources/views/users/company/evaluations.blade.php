...
@section('content')
                        ...
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="p-4 text-gray-600">
                                <div class="flex space-x-2 justify-center">
                                    ...
                                    <!-- Botão Avaliar -->
                                    <button type="button" onclick="avaliateModal({{ $final_report['id'] }}, '{{ $final_report['company_evaluation'] }}')" class="bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-2 rounded flex items-center">
    ...
    <!-- Modal de Avaliar -->
    <div id="avaliateModal" class="fixed inset-0 items-center sm:h-screen justify-center z-50 bg-black bg-opacity-50 hidden text-sm">
        <div class="bg-white rounded-lg shadow-lg p-6 w-11/12 sm:w-3/4 md:w-2/3 lg:w-1/2">
            ...
            <!-- Form -->
            <form id="avaliateModal" action="{{ route('company.evaluations.store', $final_report['id']) }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="flex justify-end">
                    <button type="button" class="bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-2 xl:px-4 rounded mr-2" onclick="closeModal('avaliateModal')">Cancelar</button>
    ...
    <script>
        ...
        function avaliateModal(id,companyEvaluation) {
            openModal('avaliateModal');

            document.getElementById('update_company_evaluation').value = companyEvaluation;

            const avaliateModal = document.getElementById('avaliateModal');
            avaliateModal.action = `/company/evaluations/${id}`;
        }
